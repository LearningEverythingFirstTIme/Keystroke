...
A system-wide AI autocomplete for Windows. Keystroke runs in the background, watches what you type in any application, and suggests completions powered by Google Gemini.

![.NET 8](https://img.shields.io/badge/.NET-8.0-purple) ![Windows](https://img.shields.io/badge/platform-Windows-blue) ![Gemini](https://img.shields.io/badge/AI-Gemini%202.5-orange)

## How it works

1. A low-level keyboard hook captures your keystrokes across all applications
2. After a brief debounce, your typed text is sent to Google Gemini along with context from the active window
3. A suggestion panel appears near your cursor with the predicted completion
4. Press **Tab** to accept, **Esc** to dismiss, or just keep typing

## Features

- **System-wide** — works in any text field (browsers, chat apps, editors, email clients, etc.)
- **Streaming predictions** — suggestions appear progressively as Gemini responds
...
- **Configurable** — settings UI for model, temperature, debounce timing, completion length, OCR toggle, and system prompt
...
The exe will be at `publish/KeystrokeApp.exe`.

### First-time setup

1. Launch the app — it runs in the system tray (look for the keyboard icon)
...
|---|---|---|
| Engine | Gemini | Prediction engine (Gemini or Dummy for testing) |
